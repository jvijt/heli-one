...
final class Auth
{
    public static function ensureUserSchema(): void
    {
        ...
        try {
            ...
            foreach ($admins as $admin) {
                $insert->execute(['name'=>$admin['name'],'email'=>strtolower((string)$admin['email']),'password_hash'=>$admin['password_hash'],'role'=>((int)$admin['id']===$firstAdminId?'superadmin':'admin'),'is_active'=>(int)$admin['is_active'],'last_login_at'=>$admin['last_login_at']]);
            }
        } catch (Throwable) {}
        $done = true;
    }
    ...
    public static function requireLogin(): void
    {
        ...
        // Uitzondering voor leden: alleen hun eigen badge mag via badge.php worden bekeken/geprint.
        $script=basename((string)($_SERVER['SCRIPT_NAME']??''));
        if($script==='badge.php' && self::role()==='member' && (int)($_GET['id']??0)===self::memberId() && self::memberId()>0) return;
    ...
    {
        self::requireAuthenticated();
        if (!self::isSuperAdmin()) { http_response_code(403); exit('Geen toegang.'); }
    }
    ...
    public static function attempt(string $email, string $password): bool
    {
        ...
        $stmt = Database::connection()->prepare('SELECT id, member_id, email, password_hash, name, role, is_active FROM users WHERE LOWER(email) = :email LIMIT 1');
        $stmt->execute(['email' => strtolower(trim($email))]);
        $user = $stmt->fetch();
        if (!$user || !(bool)$user['is_active'] || !password_verify($password, $user['password_hash'])) return false;
        session_regenerate_id(true);
        $_SESSION['user_id']=(int)$user['id'];$_SESSION['user_name']=(string)$user['name'];$_SESSION['user_email']=(string)$user['email'];$_SESSION['user_role']=(string)$user['role'];$_SESSION['member_id']=(int)($user['member_id']??0);
        if (in_array($user['role'], ['admin','superadmin'], true)) { $_SESSION['admin_id']=(int)$user['id'];$_SESSION['admin_name']=(string)$user['name'];$_SESSION['admin_email']=(string)$user['email']; }
        else unset($_SESSION['admin_id'],$_SESSION['admin_name'],$_SESSION['admin_email']);
