...
function merge_dup_members(PDO $pdo,int $keepId,int $removeId):void {
    ...
    foreach($src->fetchAll() as $r){
        ...
        $type=($dst['membership_type']==='flying'||$r['membership_type']==='flying')?'flying':'supporting';
        $pay=($dst['payment_status']==='paid'||$r['payment_status']==='paid')?'paid':'unpaid';
        $paid=$dst['paid_at']?:$r['paid_at']; $ent=max((int)$dst['free_flight_entitled'],(int)$r['free_flight_entitled']); $flight=$dst['free_flight_date']?:$r['free_flight_date'];
