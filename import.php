...
function bool_value(mixed $value): int
{
    ...
    return in_array($v, ['1','ja','yes','true','y','x','recht','j'], true) ? 1 : 0;
}
...
function member_number(PDO $pdo, int $id): void
{
    ...
    $pdo->prepare('UPDATE members SET member_number=:n WHERE id=:id AND (member_number IS NULL OR member_number="")')
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'upload') {
    ...
    try {
        if (!isset($_FILES['file']) || ($_FILES['file']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            throw new RuntimeException('Selecteer een Excel- of CSV-bestand.');
        }
        ...
        $_SESSION['member_import'] = $state = [
            'path'=>$path,'original'=>$original,'headers'=>$headers,'row_count'=>count($rows)-1,
            'preview'=>array_slice($rows,1,5)
        ];
        $pdo->prepare('INSERT INTO import_jobs (admin_id,original_filename,status,total_rows) VALUES (:admin,:file,"mapping",:total)')
            ->execute(['admin'=>(int)$_SESSION['admin_id'],'file'=>$original,'total'=>count($rows)-1]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'execute') {
    ...
    try {
        ...
        foreach (['first_name','last_name','email'] as $required) {
            if (count(array_keys($mapped, $required, true)) !== 1) throw new RuntimeException('Koppel exact één kolom aan Voornaam, Familienaam en E-mail.');
        }
        ...
        foreach ($rows as $rowIndex => $row) {
            ...
            if ($first === '' || $last === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $skipped++; $errorDetails[] = 'Rij '.$line.': voornaam, familienaam of geldig e-mailadres ontbreekt.'; continue;
            }
        ...
        if ($jobId) {
            $pdo->prepare('UPDATE import_jobs SET status="completed",imported_rows=:i,skipped_rows=:s,error_rows=:e,field_mapping=:m,completed_at=NOW() WHERE id=:id')
                ->execute(['i'=>$created+$updated,'s'=>$skipped,'e'=>$errors,'m'=>json_encode(['mapping'=>$mapping,'defaults'=>$defaults,'batch_tags'=>$batchTags],JSON_UNESCAPED_UNICODE),'id'=>$jobId]);
        }
        @unlink($state['path']);
        unset($_SESSION['member_import']);
        $state=null;
...
<?php else:?><form method="post" id="importForm"><input type="hidden" name="csrf_token" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="execute"><div class="card"><h2>2. Kolommen koppelen</h2><p><strong><?=e($state['original'])?></strong> · <?= (int)$state['row_count'] ?> gegevensrijen</p><div class="mapping"><table><thead><tr><th>Excel-kolom</th><th>Heli One-veld</th><th>Voorbeeld</th></tr></thead><tbody><?php foreach($state['headers'] as $i=>$header):?><tr><td><strong><?=e($header)?></strong></td><td><select name="mapping[<?=$i?>]"><?php foreach($fieldLabels as $key=>$label):?><option value="<?=e($key)?>"><?=e($label)?></option><?php endforeach;?></select></td><td><?=e((string)($state['preview'][0][$i] ?? ''))?></td></tr><?php endforeach;?></tbody></table></div><p class="muted">Voornaam, Familienaam en E-mail moeten elk exact één keer gekoppeld worden.</p></div>
...
<script>(()=>{const box=document.getElementById('tagbox'),input=document.getElementById('tagInput'),hidden=document.getElementById('batchTags');let tags=[];function sync(){hidden.value=tags.join('\n')}function add(v){v=v.trim();if(!v||tags.some(t=>t.toLowerCase()===v.toLowerCase()))return;tags.push(v);const c=document.createElement('span');c.className='chip';c.textContent=v+' ';const b=document.createElement('button');b.type='button';b.textContent='×';b.onclick=()=>{tags=tags.filter(t=>t!==v);c.remove();sync()};c.appendChild(b);box.insertBefore(c,input);sync()}input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();add(input.value);input.value=''}});document.querySelectorAll('.suggestion').forEach(b=>b.onclick=()=>add(b.dataset.tag||''));document.getElementById('importForm').addEventListener('submit',()=>{if(input.value.trim())add(input.value)});})();</script><?php endif;?></main></body></html>