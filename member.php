...
$member = [
    ...
    'member_since' => date('Y-m-d'), 'member_type' => 'viewer', 'status' => 'active', 'photo_path' => '', 'notes' => ''
];
$membership = ['payment_status' => 'unpaid', 'paid_at' => '', 'free_flight_entitled' => 0, 'free_flight_date' => ''];
$tagNames = [];
...
function load_member(PDO $pdo, int $id, int $year): array
{
    ...
    $defaultEntitled = (($m['member_type'] ?? 'viewer') === 'flyer') ? 1 : 0;
    $membership = $ms->fetch() ?: ['payment_status' => 'unpaid', 'paid_at' => '', 'free_flight_entitled' => $defaultEntitled, 'free_flight_date' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $memberType = in_array($_POST['member_type'] ?? '', ['viewer','flyer','pilot'], true) ? (string)$_POST['member_type'] : 'viewer';
    ...
    $freeFlightEntitled = $memberType === 'flyer' ? 1 : 0;
    ...
    elseif ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) $error = 'Vul een geldig e-mailadres in.';
        ...
        try {
            ...
            $freeFlightDate=$memberType==='flyer' ? (($_POST['free_flight_date']??'')?:null) : null;
